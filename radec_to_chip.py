...
def radec_to_chip(obsRA, obsDec, obsPA, ptRA, ptDec):
    ...
    mX = -np.sin(obsDec) * np.cos(ptDec[begin:end]) * np.cos(obsRA - ptRA[begin:end]) + np.cos(
        obsDec
    ) * np.sin(ptDec[begin:end])
    ...
    for i in range(18):
        ...
        mask = (
            (cptr[0 + 12 * i] * xi + cptr[1 + 12 * i] * yi < cptr[2 + 12 * i])
    ...
    return np.pad(SCA, (begin, len(ptDec) - end), "constant", constant_values=(0, 0))[
        np.argsort(sort)
    ]  # Pad SCA array with zeros and resort to original indexing
